...
if (isset($_POST['nick_id'])) {
	$nick_id = $_POST['nick_id'];
}
...
$sql="SELECT `id`, `post_brief`, `title`, `nickname`, `email`, `nickname_sys`, `nick_id`, `created`,`likes`, `dislikes`, `abusive` FROM post_metas where `nick_id`=? and id between ? and ? ORDER BY id desc";
...
if($last_id==0)
{
	$sql="SELECT `id`, `post_brief`, `title`, `nickname`, `email`, `nickname_sys`, `nick_id`, `created`,`likes`, `dislikes`, `abusive` FROM post_metas where `nick_id`=? ORDER BY id desc limit 20";
}
...
{	
	...
	{
		$para='iii';
		$stmt->bind_param($para,$nick_id,$start,$end); //bind variable to prepared stmt
	}else{
		$para='i';
		$stmt->bind_param($para,$nick_id); //bind variable to prepared stmt
	}
//	echo $sql.$para;
	//$stmt->bind_param($para,$start,$end); //bind variable to prepared stmt
		...
		{	
			...
			{	
				...
				echo "<div class='content_paragraphs_individual'>";
					...
					echo "<div style='clear:both'>";
					if($nickname!='')
					{	echo "<span class='span_left'>"."--".stripslashes($nickname)."&nbsp;</span>";
